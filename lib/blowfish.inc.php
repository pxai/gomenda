 ...
 * Utilzado para phpframework con ciertos cambios para permitir
 * cifrado de strings de tamaño indeterminado.
 */
//class Horde_Cipher_blowfish extends Horde_Cipher {
// la llamamos simplemente EncryptionAlgorithm
...
/**
* funcion arraydump
* Todo el contenido del array lo suma en un unico string
*/
function arrayDump ($arreglo) {
	$resultado = "";
...
/**
* funcion fillString
* blowfish solo cifra de 8 en 8 asi que esta funcion prepara el string para
* que sea divisible por 8 aÃ±adiendo espacios en blanco
*/
function fillString (&$cadena) {
	$cuanto = 8 - (strlen($cadena) % 8);
	for ($i=0;$i<$cuanto;$i++) 
		$cadena .= " ";
}
...
* Se le pasa un string y devuelve un string cifrado y encodeado a base64
*/
function  encrypt($cadena, $new_key = "") {

	// rellenamos el string
	$this->fillString($cadena);
	...
	// convertimos el string en un array de string cifrados
	for ($i=0;$i<strlen($cadena);$i+=8) {
		$j = $i + 7;
		//echo "[$i-" .($i+7). "] ". substr($cadena,$i,8)." <br>"; // DEBUG
		$bloque_cifrado[] = base64_encode($this->encryptBlock(substr($cadena,$i,8)));
		//echo "Bloque $i: ". $bloque_cifrado[($i/8)] . " , tam: " .strlen($bloque_cifrado[($i/8)]). "<br>"; // DEBUG
	
	}

	return $this->arrayDump($bloque_cifrado);
...
* Se le pasa un string cifrado y encodeado a 64 y devuelve el string plano pasado por trim
* para restarle los espacios en blanco.
* 
*/
function  decrypt ($cadena, $new_key = "") {
	...
	for ($i=0;$i<strlen($cadena);$i+=12) {
		$j = $i + 11;
		//echo "[$i-" .($i+11). "] ". substr($cadena,$i,12)." <br>"; // DEBUG
		$bloque_descifrado[] = $this->decryptBlock(base64_decode( substr($cadena, $i, 12) ));
	}

	return trim($this->arrayDump($bloque_descifrado));
